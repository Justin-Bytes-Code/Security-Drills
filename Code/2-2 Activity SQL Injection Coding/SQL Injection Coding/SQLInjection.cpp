...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  std::string check = sql; 
  std::transform(check.begin(), check.end(), check.begin(), ::tolower);

  // detect or = as possible SQL injection
  if (check.find(" or ") != std::string::npos && check.find("=") != std::string::npos) {
      std::cout << "\n" << "\033[31mInjection is Detected\033[0m" << std::endl;
      return false;
