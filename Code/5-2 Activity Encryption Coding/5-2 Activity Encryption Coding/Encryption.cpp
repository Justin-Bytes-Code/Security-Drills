...
std::string encrypt_decrypt(const std::string& source, const std::string& key)
{
  ...
  { // TODO: DONE student need to change the next line from output[i] = source[i]
...
std::string read_file(const std::string& filename)
{
  ...
  // TODO: DONE implement loading the file into a string
...
{
  //  TODO: DONE implement file saving
  //  file format
  //  Line 1: student name
  ...
  //  Line 4+: data
    ...
    ctime_s(str, size, &current_time);

    std::ofstream decrypted_file("decrytpteddatafile.txt");

    decrypted_file << student_name << "\n" << str << key << "\n" << data;
